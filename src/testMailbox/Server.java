			...
			Command login = new Command() {
				...
				public StatusAndData execute(String[] para, Socket client) {
					...
					if (server.getUsers().size() >= server.getMAXUSERS()) {
						information = new StatusAndData(
								StatusCodes.SERVICEUNAVAILABLE,
								new String[] { "Maximal allowed count of users reached." });
						return information;
					}
					if (!checkUsernameFree(para[0])) {
						information = new StatusAndData(
								StatusCodes.SERVICEUNAVAILABLE,
								new String[] { "Username already taken" });
						return information;
					}
					users.add(new User(para[0], client));
					return new StatusAndData(StatusCodes.OKMITANTWORT,
			...
			Command logout = new Command() {
				...
				public StatusAndData execute(String[] para, Socket client) {
					...
					if (!checkLoggedIn(client)) {
						information = new StatusAndData(
								StatusCodes.NICHTEINGELOGGT,
								new String[] { "You are not logged in." });
						return information;
					}
					...
					return new StatusAndData(
							StatusCodes.OKMITANTWORT,
							new String[] { "You've been successfully logged out" });
				}
			};
		...
		case "ls":
			Command ls = new Command() {
				...
				public StatusAndData execute(String[] para, Socket client) {
					...
					if (file.list() == null) {
						return new StatusAndData(
								StatusCodes.FEHLRHAFTEANFRAGE,
								new String[] { "File with this path doesn't exist" });
					}
					return new StatusAndData(StatusCodes.OKMITANTWORT,
							file.list());
				}
			...
			Command chat = new Command() {
				...
				public StatusAndData execute(String[] para, Socket client) {
					...
					if (checkUsernameFree(para[0])) {
						information = new StatusAndData(
								StatusCodes.FEHLRHAFTEANFRAGE,
								new String[] { "There isn't a user with this username" });
						return information;
					}
					User contact = null;
		...
		case "notes":
			Command notes = new Command() {
				...
				public StatusAndData execute(String[] para, Socket client) {
					...
					return new StatusAndData(
							StatusCodes.OKMITANTWORT,
							noteMessages.toArray(new String[noteMessages.size()]));
				}
			};
			info = notes.execute(request.getParams(), client);

		}
		return new Response(info.getStatus(), request.getSequence(),
				info.getData());
		...
		new Thread(new Runnable() {
			...
			public void run() {
				while (true) {
					...
					try {
						synchronized (this) {// this bezieht sich hier auf das
												// runnable
							this.wait(1000);
						}
